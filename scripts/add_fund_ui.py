...
fund_html = r'''
    <section class="panel fund-panel" id="fundPanel">
      <div class="fund-head">
        <div><h3>板块代表基金</h3><p>每个板块展示 5 只公开交易 ETF；涨跌与最大回撤随上方固定周期切换。</p></div>
        <div class="fund-period" id="fundPeriod">—</div>
      </div>
      <div class="fund-table-wrap" id="fundTableWrap"><div class="fund-empty">正在读取基金数据…</div></div>
      <p class="fund-note" id="fundNote">基金为主题相关的代表性样本，不构成推荐或排名。涨跌与回撤按交易所公开复权价格计算；最大回撤指所选周期内从阶段高点到随后低点的最大跌幅。</p>
    </section>
...
function renderFunds(item){const funds=(item?.funds||[]).slice(0,5),wrap=$('#fundTableWrap'),period=$('#fundPeriod'),note=$('#fundNote');period.textContent=activePeriod+' 指标';if(customRange){note.textContent=`当前图表为自定义日期区间；基金表仍按“${activePeriod}”固定周期显示。基金为主题相关的代表性样本，不构成推荐或排名。最大回撤按所选固定周期内公开复权价格计算。`}else{note.textContent='基金为主题相关的代表性样本，不构成推荐或排名。涨跌与回撤按交易所公开复权价格计算；最大回撤指所选周期内从阶段高点到随后低点的最大跌幅。'}if(!funds.length){wrap.innerHTML='<div class="fund-empty">该板块暂未生成代表基金数据</div>';return}const rows=funds.map(f=>`<tr><td><span class="fund-name">${esc(f.name||'—')}</span><span class="fund-code">${esc((f.code||'').replace(/^(sh|sz)/,''))}</span></td><td>${fundFmt(f.returns?.[activePeriod])}</td><td>${drawdownFmt(f.drawdowns?.[activePeriod])}</td><td>${esc(f.latest_date||'—')}</td><td>${esc(f.start_date||'—')}</td></tr>`).join('');wrap.innerHTML=`<table class="fund-table"><thead><tr><th>基金</th><th>${esc(activePeriod)}涨跌</th><th>最大回撤</th><th>数据截至</th><th>历史起点</th></tr></thead><tbody>${rows}</tbody></table>`}
'''
js_marker = 'function renderDetail(){'
if js_marker not in text:
    raise RuntimeError("JS marker missing")
...
print("[done] fund panel UI added")
